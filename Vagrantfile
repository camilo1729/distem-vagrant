...
#      node.vm.network :private_network, ip: ipAddrPrefix + num.to_s, virtualbox__intnet: "tap#{num}"
      ip_vnode = ipAddrPrefix + num.to_s
      node.vm.network "private_network", ip: ip_vnode
      #node.vm.network :public_network, ip: ip_vnode, bridge: "tap#{num}"
      #node.vm.network :forwarded_port, guest: 22, disabled: true
      node.vm.provider :virtualbox do |v|
        v.name = "Compute Node" + num.to_s
      ...
      node.vm.provision :shell, inline: "gem install pry-remote"
      node.vm.provision :shell, inline: "apt-get update"
      # we install pry-stack explorer in the main node
      node.vm.provision :shell, inline: "DEBIAN_FRONTEND=noninteractive apt-get install -q -y ruby-dev emacs" if num==1
      node.vm.provision :shell, inline: "gem install pry-stack_explorer" if num==1
    end
  end
